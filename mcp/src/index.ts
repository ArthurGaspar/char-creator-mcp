...
function charDir(name: string) {
  return join(CHARACTERS_DIR, name.toLowerCase());
}

function sectionFile(name: string, section: Section) {
  return join(charDir(name), `${section}.md`);
}

function compiledFile(name: string) {
...
function buildCompiled(name: string): string {
  const dir = charDir(name);
  if (!existsSync(dir)) throw new Error(`Character "${name}" not found.`);
...
const TOOLS: Tool[] = [
  ...
  },
  {
    ...
    description:
      "Compile all sections of a character. Without synthesize: merges sections and saves compiled.md directly. With synthesize: returns the raw merged sections so the AI caller can rewrite them into a cohesive identity document, then save via save_compiled.",
    inputSchema: {
      type: "object",
      properties: {
        character: { type: "string" },
  ...
  try {
    switch (name) {
      ...
      case "create_character": {
        const { name: charName } = args as { name: string };
        const dir = charDir(charName);
        if (existsSync(dir)) return err(`"${charName}" already exists.`);
      ...
      case "get_section": {
        const { character, section } = args as { character: string; section: Section };
        const path = sectionFile(character, section);
        if (!existsSync(path)) return err(`Section "${section}" not found for "${character}".`);
        return text(readFileSync(path, "utf-8"));
      }

      case "set_section": {
        const { character, section, content } = args as { character: string; section: Section; content: string };
        if (!existsSync(charDir(character))) return err(`"${character}" does not exist. Use create_character first.`);
      ...
      case "save_compiled": {
        const { character, content } = args as { character: string; content: string };
        if (!existsSync(charDir(character))) return err(`"${character}" does not exist.`);
      ...
      case "get_compiled": {
        ...
        if (!existsSync(path)) return err(`No compiled file for "${character}". Run compile_character first.`);
        return text(readFileSync(path, "utf-8"));
      }

      case "delete_character": {
        const { character, confirm } = args as { character: string; confirm: boolean };
        if (!confirm) return err("Set confirm: true to proceed. This cannot be undone.");
        if (!existsSync(charDir(character))) return err(`"${character}" not found.`);
